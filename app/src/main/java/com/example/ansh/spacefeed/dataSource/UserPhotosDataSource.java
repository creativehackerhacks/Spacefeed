...
public class UserPhotosDataSource extends PageKeyedDataSource<Integer, Photo> {
    ...
    public UserPhotosDataSource(Executor executor, String username, String userPhotoSortOrder) {
        mUnsplashService = mUnsplashService.getInstance();
        this.mUsername = username;
        this.mUserPhotoSortOrder = userPhotoSortOrder;
    }

    @Override
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, Photo> callback) {
        mUnsplashService.getApi().getUserPhotos(mUsername, CLIENT_ID, params.requestedLoadSize, 1, mUserPhotoSortOrder)
    ...
    public void loadAfter(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, Photo> callback) {
        mUnsplashService.getApi().getUserPhotos(mUsername, CLIENT_ID, params.requestedLoadSize, params.key, mUserPhotoSortOrder)
                .enqueue(new Callback<List<Photo>>() {
                    @Override
                    public void onResponse(Call<List<Photo>> call, Response<List<Photo>> response) {
                        if(response.isSuccessful() && response.code() == 200) {
