...
public class UserLikesDataSource extends PageKeyedDataSource<Integer, Photo> {
    ...
    public UserLikesDataSource(Executor executor, String username, String userLikesSortOrder) {
        mUnsplashService = mUnsplashService.getInstance();
        this.mUsername = username;
        this.mUserLikesSortOrder = userLikesSortOrder;
    }
    ...
    public void loadInitial(@NonNull PageKeyedDataSource.LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, Photo> callback) {
        mUnsplashService.getApi().getUserLikes(mUsername, CLIENT_ID, params.requestedLoadSize, 1, mUserLikesSortOrder)
    ...
    public void loadAfter(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, Photo> callback) {
        mUnsplashService.getApi().getUserLikes(mUsername, CLIENT_ID, params.requestedLoadSize, params.key, mUserLikesSortOrder)
                .enqueue(new Callback<List<Photo>>() {
                    @Override
                    public void onResponse(Call<List<Photo>> call, Response<List<Photo>> response) {
                        if(response.isSuccessful() && response.code() == 200) {
